    ...
    def generate(self,
        ...
        while True:
            ...
            for i in range(len(batch_X)):
                ...
                # batch_X[i] = cv2.resize(batch_X[i], dsize=(256, 256))
                batch_gray.append(batch_X[i])
                batch_gray[i] = cv2.cvtColor(batch_gray[i], cv2.COLOR_BGR2GRAY)
                batch_gray[i] = batch_gray[i].reshape(batch_gray[i].shape[0], batch_gray[i].shape[1],1)
            ...
            if 'gray_image' in returns: ret.append(batch_gray)
