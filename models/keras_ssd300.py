...
def ssd_300(image_size,
    ...
    def grayscale_to_rgb(tensor, channel_axis=-1):
        # images= K.expand_dims(images, axis=channel_axis)
        tiling = [1] * 4    # 4 dimensions: B, H, W, C
        tiling[channel_axis] *= 3
        tensor= K.tile(tensor, tiling)
        return tensor
    ...
    ############################################################################
    """
This If it's a Keras tensor, then it means it is connected to a layer/model and from there it can take values; therefore, using it as it is would be fine. However, if it's not a Keras tensor it should be wrapped in a Input layer so that it could be fed (i.e. take values) using a placeholder (which is the underlying structure of Input layer)
        if not K.is_keras_tensor(input_tensor):
            x = layers.Input(tensor=input_tensor)
        else:
            x = input_tensor
"""
