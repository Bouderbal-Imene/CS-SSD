...
from keras_layers.preprocess import _preprocess_conv2d_input, _preprocess_padding
from tensorflow.keras.layers import InputSpec, Layer
import tensorflow as tf
from tensorflow.keras.layers import Conv2D
from keras_layers.interfaces import conv2d_args_preprocessor, generate_legacy_interface
...
class DepthwiseConv2D(Conv2D):
    ...
    def compute_output_shape(self, input_shape):
        ...
        rows = K.utils.conv_utils.conv_output_length(rows, self.kernel_size[0],
                                             self.padding,
                                             self.strides[0])
        cols = K.utils.conv_utils.conv_output_length(cols, self.kernel_size[1],
                                             self.padding,
                                             self.strides[1])
        if self.data_format == 'channels_first':
